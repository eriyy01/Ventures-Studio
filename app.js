...
// booking line stays honest: show next calendar month automatically
const bm=$("#bookingMonth");if(bm){try{const d=new Date();d.setMonth(d.getMonth()+1);bm.textContent=d.toLocaleString("en-US",{month:"long"});}catch(e){}}
...
cf.innerHTML='';const done=document.createElement("div");done.className="ok-msg";done.setAttribute("role","status");const hi=document.createElement("strong");hi.textContent="Thanks, "+data.name.split(" ")[0]+". Your email app should have opened.";done.appendChild(hi);done.appendChild(document.createElement("br"));done.appendChild(document.createTextNode("Hit send there and Maya replies within two business days. Prefer to write directly? "));const ml=document.createElement("a");ml.href="mailto:hello@venturestudios.co";ml.textContent="hello@venturestudios.co";done.appendChild(ml);done.appendChild(document.createTextNode(". To enable one-click sending, connect a form endpoint (see data-formspree)."));cf.appendChild(done);}
